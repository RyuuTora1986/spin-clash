...
6. Confirm `Peak Ram` remains unlocked.
7. In Quick Battle, select locked `Shard Hex Array` without enough SCRAP and confirm trial flow.
8. In a funded save, permanently unlock `Shard Hex Array`.
...
7. In `日本語`, confirm Home / Loadout / Quick Battle selection / Path preview names show small hiragana ruby above top and arena proper names.
8. Start a match and confirm the HUD fixed labels, intro banner, and result text use the selected language.
9. Trigger `SHARE` and confirm the browser share text or exported result-card copy uses the selected language.
10. Refresh the page.
11. Confirm the last selected language is restored automatically.
12. If possible, repeat one pass in each of the three languages and watch for clipping, overlap, or broken button text.
...
- Japanese ruby appears only on the approved preview/loadout surfaces and does not leak into HUD, result, or share text
